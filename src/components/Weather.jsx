...
export default function Weather(props){
    ...
    return (
        <div>
        <span className="weatherCheck" onClick={()=>props.getWeatherData(lati,long,0)}>Check Weather Here</span>
        <div className="weather" >
            <div>
                <div className="day-weather" onClick={()=>props.getWeatherData(lati,long,0)}>1DAY</div>
                <div>{props.weather0}</div>
                <div>Highest {props.tempHigh0}℃</div>
                <div>Lowest {props.tempLow0}℃</div>
            </div>
            <div>
                <div className="day-weather" onClick={()=>props.getWeatherData(lati,long,1)}>2DAYS</div>
                <div>{props.weather1}</div>
                <div>Highest {props.tempHigh1}℃</div>
                <div>Lowest {props.tempLow1}℃</div>
            </div>
            <div>
                <div className="day-weather" onClick={()=>props.getWeatherData(lati,long,2)}>3DAYS</div>
